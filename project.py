...
class help_button(View):
    ...
    async def button2(self, button, interaction : Interaction):
        ...
        embed2.add_field(name="🦦Fun Commands:", value=" `8ball` `waifu` `neko` `smug` `pet` `feed` `hug` `slap` `tickle` `meow` `lizard` `baka`",inline=False)
...
@bot.slash_command(name="image_generation", description="write a prompt to convert it into image")
async def imagine(ctx : Interaction, prompt: str, negative_prompt : str = " "):
    view = downloadview()
    ...
    try:
        ...
        view.add_item(Button(label="Download",url=output[0],style=nextcord.ButtonStyle.link,emoji="⬇️"))
